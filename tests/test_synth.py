"""neuralgeom.synth test: generation, embedding, the ground-truth contract, and the
...
def test_generate_gaussian():
    d = syn.generate("input", syn.INPUT_COND, n_trials=12, seed=0, bin_s=0.05)
...
def test_generate_poisson_matches_count():
    d = syn.generate("landscape", syn.LAND_MATCHED, n_trials=10, seed=1, bin_s=0.05,
...
def test_both_mechanisms_run():
    for mech, lv in (("input", syn.INPUT_COND), ("landscape", syn.LAND_AY2)):
...
def test_round_trip_to_session():
    d = syn.generate("input", syn.INPUT_COND, n_trials=6, seed=3, bin_s=0.05)
    s = from_synthetic(d)
...
def test_dynamics_independent_of_synth():
    ...
    reach the estimators through the same Session contract, never through the
