...
import pytest
import torch
from neuralgeom.tasks.models import VanillaRNN
from neuralgeom.tasks.timing.config import ObservationConfig, SchedulerConfig, TimingTaskConfig
from neuralgeom.tasks.timing.generator import TrialGenerator
from neuralgeom.tasks.timing.rl import ActorCritic, RLTrainer, rollout, summarise
from neuralgeom.tasks.timing.scheduler import DelayScheduler
...
def test_value_gain_scales_the_water_and_the_early_penalty_together():
    ...
    from neuralgeom.tasks.timing.config import TimingTaskConfig
    base, scaled = [], []
    for gain in (0.0, 1.0):
...
def test_run_report_writes_every_figure(tmp_path):
    import matplotlib
    matplotlib.use("Agg")
    from neuralgeom.tasks.timing.plots import run_report
    tr, m = build(n_envs=4, iti_restart_on_lick=False)
    h = tr.train(m, steps=2, log_every=1, verbose=False)
...
def test_history_regression_null_brackets_a_shuffled_effect():
    ...
    from neuralgeom.tasks.timing.plots import history_regression
    rng = np.random.default_rng(0)
...
def test_history_dashboard_makes_six_panels(tmp_path):
    import matplotlib
    matplotlib.use("Agg")
    from neuralgeom.tasks.timing.plots import history_dashboard
    tr, m = build(n_envs=4, iti_restart_on_lick=False)
...
def test_model_figure_uses_the_hidden_states(tmp_path):
    import matplotlib
    matplotlib.use("Agg")
    from neuralgeom.tasks.timing.plots import _model_figure
    tr, m = build(n_envs=4, iti_restart_on_lick=False)
...
def test_heads_dashboard_renders(tmp_path):
    import matplotlib
    matplotlib.use("Agg")
    from neuralgeom.tasks.timing.plots import heads_dashboard
    tr, m = build(n_envs=4, iti_restart_on_lick=False)
    h = tr.train(m, steps=3, log_every=1, verbose=False)
...
def test_time_penalty_is_not_charged_during_the_post_lick_winddown():
    ...
    from neuralgeom.tasks.timing.generator import Phase
...
def test_every_wrong_lick_costs_the_same_in_base():
    from neuralgeom.tasks.timing.variants import BASE
    t = BASE["task"]
...
def test_describe_is_generated_from_the_live_config():
    from neuralgeom.tasks.timing.variants import describe, BASE
    md = describe("act")
