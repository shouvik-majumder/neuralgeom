# Timing task — configuration reference (`act_both_noclip`)

Inheritance: `BASE` -> `act` -> `act_both_noclip`.

Generated 2026-09-04 by `python -m neuralgeom.tasks.timing.variants --describe`. Do not edit by hand; regenerate.
...
| `cue_mode` | `'both'` | pulse (transient only) | step (tonic, held to trial end) | both (Yang et al.: a transient cue AND a tonic step) **changed** |
...
| `grad_clip` | `0.0` | gradient-norm clip (0 = off) **changed** |
