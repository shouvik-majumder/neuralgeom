"""
neuralgeom.tasks.timing.variants -- named training configurations.
==================================================================
...
Usage
-----
    python -m neuralgeom.tasks.timing.variants G --steps 400
    python -m neuralgeom.tasks.timing.variants all --steps 400 --out runs/

or from a notebook::

    from neuralgeom.tasks.timing.variants import run, RUNS
...
def _make_core(input_size: int, hidden: int, tau, noise: float,
    ...
    try:
        ...
        from neuralgeom.tasks.models import VanillaRNN         # type: ignore
...
def describe(name: str = "act") -> str:
    ...
    L = [f"# Timing task — configuration reference (`{name}`)", "",
         ...
         f"`python -m neuralgeom.tasks.timing.variants --describe`. "
